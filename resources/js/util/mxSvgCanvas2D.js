...
mxSvgCanvas2D.prototype.createDiv = function (
  ...
  return mxUtils.parseXml(
    `<div xmlns="http://www.w3.org/1999/xhtml" style="${style}">${val}</div>`
  ).documentElement;
};
...
mxSvgCanvas2D.prototype.text = function (
  ...
  if (this.textEnabled && str != null) {
    ...
    if (this.foEnabled && format == "html") {
      ...
      if (clip) {
        style += `overflow:hidden;max-height:${Math.round(
          h
        )}px;max-width:${Math.round(w)}px;`;
      } else if (overflow == "fill") {
        style += `width:${Math.round(w + 1)}px;height:${Math.round(
          h + 1
        )}px;overflow:hidden;`;
      } else if (overflow == "width") {
      ...
      group.setAttribute(
        "transform",
        `translate(${Math.round(x) + this.foOffset},${
